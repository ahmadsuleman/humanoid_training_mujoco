...
class JointCPG:
    """
    Simple Hopf-style CPG for joint-space smoothing.
    Each joint has an independent oscillator.
    """

    def __init__(self, n_joints: int, dt: float, omega: float = 3.0, alpha: float = 10.0):
        self.n = n_joints
        self.dt = dt
        self.omega = omega      # base frequency
        self.alpha = alpha      # convergence rate

        # Oscillator state
        self.x = np.zeros(n_joints, dtype=np.float32)
        self.y = np.zeros(n_joints, dtype=np.float32)

    def reset(self):
        self.x[:] = 0.1 * np.random.randn(self.n)
        self.y[:] = 0.0

    def step(self, modulation: np.ndarray) -> np.ndarray:
        """
        modulation: RL action (shape [n_joints])
        returns: CPG-filtered joint delta
        """
        r2 = self.x**2 + self.y**2

        dx = self.alpha * (1.0 - r2) * self.x - self.omega * self.y
        dy = self.alpha * (1.0 - r2) * self.y + self.omega * self.x

        self.x += dx * self.dt
        self.y += dy * self.dt

        # RL modulates oscillator amplitude
        return modulation * self.x
...
class G1ArmReachEnv(gym.Env):
    ...
    def __init__(self, render_mode: str | None = None, right_arm: bool = False):
        ...
        # self._floor_colours = [
        #     (97 / 255.0, 161 / 255.0, 84 / 255.0, 0.0)
        # ]
        ...
        # --------------------------------------------------------------
        #  CPG setup (optional motion prior)
        # --------------------------------------------------------------
        self._use_cpg = True  # toggle for ablations

        dt = self.model.opt.timestep
        self._cpg = JointCPG(
            n_joints=7,
            dt=dt,
            omega=3.0,   # rad/s (tunable)
            alpha=10.0
        )
    ...
    def step(self, action: np.ndarray):  # noqa: D401
        ...
        if getattr(self, "_terminated", False):
            # Don't modify state, just return last state
            obs = self._get_obs()
        ...
        # 1. integrate joint delta (RL or RL+CPG)
        if self._use_cpg:
            dq_cmd = self._cpg.step(action)
        else:
            dq_cmd = action

        for adr, dq in zip(self._qadr, dq_cmd):
            self.data.qpos[adr] = np.clip(
                self.data.qpos[adr] + float(dq),
                -3.14,
                3.14
            )
        ...
        info["action_l2"] = float(np.linalg.norm(action))
        if self._use_cpg:
            info["cpg_action_l2"] = float(np.linalg.norm(dq_cmd))
...
#     positions.append(env.data.qpos[env._qadr].copy())
#     velocities.append(env.data.qvel[env._qadr].copy())

# # Plot or print positions/velocities
